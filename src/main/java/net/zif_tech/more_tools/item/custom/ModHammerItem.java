...
public class ModHammerItem extends MiningToolItem {
    ...
    public int getSize(ItemStack stack) {
        return stack.get(ModComponents.SIZE);
    }
    ...
    public static List<BlockPos> getBlocksToBeDestroyed(BlockPos initialBlockPos, PlayerEntity player){
        ...
        if (hit.getType() == HitResult.Type.BLOCK) {
            ...
            if (!(heldItem.getItem() instanceof ModHammerItem tool)) return positions;

            Block initialBlock = player.getWorld().getBlockState(initialBlockPos).getBlock();
